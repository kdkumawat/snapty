...
export default function LandingPage() {
  ...
  const handlePasteClick = async () => {
    ...
    // empty-state paste UI when nothing is there or the API is denied.
    try {
        ...
        for (const item of items) {
          for (const type of item.types) {
            if (type.startsWith('image/')) {
              ...
              const { loadImageFileIntoEditor } = await import('@/lib/image-load');
              await loadImageFileIntoEditor(file);
              router.push('/editor');
              return;
            }
          }
        }
        toastInfo('Clipboard Empty', 'Copy an image first, then paste in the editor');
      }
    } catch {
      toastInfo('Paste in the editor', 'Press Ctrl+V once the editor is open');
    }
    router.push('/editor');
  };
    ...
    <MotionConfig reducedMotion={reducedMotion ? 'always' : 'never'}>
      <div className="relative flex-1 min-h-0 h-full w-full overflow-y-auto overflow-x-hidden bg-canvas text-foreground">
        ...
        <header className="sticky top-0 z-50 backdrop-blur-md bg-canvas/90 border-b border-border/40 flex items-center justify-between px-5 sm:px-8 py-3 isolate">
          <Link href="/" className="flex items-center gap-2.5" aria-label="Snapty home">
        ...
        <main id="main-content" className="relative">
          <section id="hero" className="flex flex-col items-center px-5 pt-10 sm:pt-16 pb-16 text-center max-w-3xl mx-auto min-h-[calc(100dvh-4rem)] justify-center scroll-mt-20">
            <motion.h1
          ...
          <section id="features" aria-labelledby="features-title" className="px-5 sm:px-8 py-20 border-t border-border/50 bg-surface/40 scroll-mt-20">
            <div className="max-w-5xl mx-auto">
              <h2 id="features-title" className="text-2xl sm:text-3xl font-semibold tracking-tight text-center mb-3">
          ...
          <section id="get-started" className="px-5 py-20 text-center scroll-mt-20">

...
const shortcuts = [
  { cat: 'General', items: [["V", "Select tool"], ["H / Space (hold)", "Hand / Pan"], ["?", "Show shortcuts"], ["Escape", "Deselect / Select tool"], [`${M}+Z`, "Undo"], [`${M}+Shift+Z`, "Redo"], ["Delete / Backspace", "Delete selected"], [`${M}+Shift+Backspace`, "Clear all annotations"], [`${M}+A`, "Select all"], [`${M}+E`, "Export"], [`${M}+C`, "Copy image to clipboard"], ["Camera icon", "Capture screen / window (then crop)"], [`${M}+= / ${M}+-`, "Zoom in / out"], [`${M}+0`, "Fit to screen"], ["Pinch", "Zoom on touch devices"]] },
